...
from lineage.models import ConnectionType, DataLevel, DataType
...
class TestNodeShapes:
    ...
    def test_node_shapes_contains_all_data_types(self):
        ...
        for data_type in data_types:
            assert data_type in NODE_SHAPES, (
    ...
    def test_node_shapes_structure(self):
        ...
        for data_type, shape in NODE_SHAPES.items():
            ...
            assert len(shape) == 2, (
                f"Shape for {data_type} must have exactly 2 elements"
            )

            prefix, suffix = shape
            assert isinstance(prefix, str), f"Prefix for {data_type} must be a string"
            assert isinstance(suffix, str), f"Suffix for {data_type} must be a string"
    ...
    def test_node_shapes_has_no_extra_keys(self):
        ...
        assert len(extra_keys) == 0, f"NODE_SHAPES contains invalid keys: {extra_keys}"
...
class TestDataLevelStyles:
    ...
    def test_data_level_styles_contains_all_data_levels(self):
        ...
        for level in data_levels:
            assert level in DATA_LEVEL_STYLES, (
    ...
    def test_data_level_styles_values_are_strings(self):
        ...
        for level, style in DATA_LEVEL_STYLES.items():
            assert isinstance(style, str), f"Style for {level} must be a string"
    ...
    def test_data_level_styles_contain_fill_property(self):
        ...
        for level, style in DATA_LEVEL_STYLES.items():
            assert "fill:" in style, f"Style for {level} must contain 'fill:' property"
...
class TestConnectionStyles:
    ...
    def test_connection_styles_contains_all_connection_types(self):
        ...
        for conn_type in connection_types:
            assert conn_type in CONNECTION_STYLES, (
    ...
    def test_connection_styles_values_are_strings(self):
        ...
        for conn_type, arrow in CONNECTION_STYLES.items():
            assert isinstance(arrow, str), f"Arrow for {conn_type} must be a string"
...
class TestStylesIntegrity:
    ...
    def test_all_configs_are_dictionaries(self):
        ...
        assert isinstance(DATA_LEVEL_STYLES, dict), (
            "DATA_LEVEL_STYLES must be a dictionary"
        )
        assert isinstance(CONNECTION_STYLES, dict), (
